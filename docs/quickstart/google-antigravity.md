...
```bash
pnpm --filter @munin/adapter-google-antigravity build
```

## 3) Lấy capabilities

```bash
pnpm --filter @munin/adapter-google-antigravity exec munin-antigravity capabilities
```

## 4) Gọi action mẫu

```bash
pnpm --filter @munin/adapter-google-antigravity exec munin-antigravity list '{"limit":10}'
```
