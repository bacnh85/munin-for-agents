...
```bash
pnpm --filter @munin/adapter-openclaw exec munin-openclaw capabilities
```

## 4) Gọi action mẫu

```bash
pnpm --filter @munin/adapter-openclaw exec munin-openclaw search '{"query":"munin ecosystem"}'
```
